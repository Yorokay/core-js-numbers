...
function getSumOfDigits(num) {
  return [...String(num)].reduce((sum, cur) => sum + Number(cur), 0);
}
...
function isPowerOfTwo(num) {
  return 2 ** Math.log2(num).toFixed(1) === num;
}
...
function getCountOfOddNumbers(number) {
  return number / 2 < 0 ? Math.ceil((number / 2) * -1) : Math.ceil(number / 2);
}
